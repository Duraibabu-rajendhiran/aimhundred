@extends('layouts.master')
@section('content')

    <div class="app-wrapper">
        <div class="app-content pt-3 p-md-3 p-lg-4">
            <div class="container-xl">
                <div class="row g-3 mb-4 align-items-center justify-content-between">
                    <div class="col-auto">
                        <h1 class="app-page-title mb-0">
                            <img width="35px" src="data:image/png;base64,"/>
                      Exam</h1>
                             </div>
                    <div class="col-auto">
                        <div class="page-utilities">
                            <div class="row g-2 justify-content-start justify-content-md-end align-items-center">
                                <div class="col-auto">
                                    
                                </div>
                            </div>
                        </div>
                    </div>
                    
                </div>
                           <style>
                           
                                        .cell {
                                            text-align: center;
                                        }
                                        .key{
                                           display:flex;
                                        }
                                        
                                        .key   a{
                                            width:100%;
                                            padding:10px;
                                        }
                                        .key a:hover{
                                            background-color:rgb(231,231,231);
                                            color:yellowgreen;
                                            font-size: 17px;
                                        }
                                       .table .app-table-hover .mb-0 .text-left{
                                            width:100% !important;
                                        }
                                    </style>
                    <div class="tab-content" id="orders-table-tab-content">
                      <div class="tab-pane fade show active" id="orders-all" role="tabpanel" aria-labelledby="orders-all-tab">
                        <div class="app-card app-card-orders-table shadow-sm mb-5">
                            <div class="app-card-body">
                                <div class="table-responsive">
                                    @if ($message = Session::get('success'))
                                        <div class="alert alert-success">
                                            <p>{{ $message }}</p>
                                        </div>
                                    @endif
                            
                                    <table class="table app-table-hover mb-0 text-left">
                                        <thead>
                                            <tr>
                                                <th class="cell">Id</th>
                                                <th class="cell">date</th>
                                                <th class="cell">class</th>
                                                <th class="cell">Action</th>
                                            </tr>
                                        </thead>
                                    <?Php if(isset($user)){
                                        $date_i = date("Y/m/d");
                                            $date1 = date('Y-m-d', strtotime($date_i . ' +0 day')); 
                                        
                                        ?>
                                        <?php $i = 1; ?>
                                         <tbody id="view1">
                                            @foreach ($user as $data)
                                                <tr >
                                                    <td class="cell">{{ $i++ }}</td>
                                                    <td class="cell">{{  date("d-m-Y", strtotime($data->date)) }}</td>
                                                    <td class="cell">{{  $data->title }}</td>
                                                    <td class="cell">
                                                        
                                                        <a href="{{route('getdateit',$data->date."-for-".$data->title)}}" class="btn btn">
                                                           view

                                            </a>
                                              <?php if($data->date >= $date1){ ?>
                                                <?php if($data->is_show == 0){ ?>

                                                   <a href="{{route('getddate',$data->title."-for-".$data->date)}}" class="btn btn">Hold</a>
                                                    
                                                    <?php }else{ ?>

                                                       <a href="{{route('getddate',$data->title."-for-".$data->date)}}" class="btn btn">
                                                     Unhold
                                                    
                                                    </a>
                                                    <?php } ?>

                                                    <a href="{{route('getddate',$data->title."-delete-".$data->date)}}"   
                                                        onclick="return confirm('you want to delete?');"
                                                        class="btn btn">
                                                     Delete
                                                    
                                                    </a>
                                                    <?php } ?>
                                              </td>
                                                 </tr>
                                            @endforeach
                                             </tbody>
                                    <?php } ?>
                                    </table>
                                </div>
                                <!--//table-responsive-->
                            </div>
                            <!--//app-card-body-->
                        </div>
                        <!--//app-card-->
                    </div>
                
                @endsection
